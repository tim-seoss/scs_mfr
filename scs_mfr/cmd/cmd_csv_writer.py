...
class CmdCSVWriter(object):
    ...
    def __init__(self):
        """stuff"""
        self.__parser = optparse.OptionParser(usage="%prog [FILENAME] [-e] [-v]", version="%prog 1.0")
    ...
    def __str__(self, *args, **kwargs):
        return "CmdCSVWriter:{filename:%s, echo:%s, verbose:%s, args:%s}" % \
                    (self.filename, self.echo, self.verbose, self.args)
