...
SYNOPSIS
gas_inference_conf.py [{ [-u UDS_PATH] [-i INTERFACE] [-s SPECIES MODEL_FILENAME] | [-r SPECIES] | -d }] [-v]
...
if __name__ == '__main__':
    ...
    if cmd.set():
        ...
        model_filenames = {} if conf is None else conf.model_filenames

        conf = GasModelConf(uds_path, model_interface, model_filenames)
        conf.save(Host)

    if cmd.set_species:
        conf.set_filename(cmd.set_species, cmd.set_filename)
        conf.save(Host)

    if cmd.remove_species:
        conf.delete_filename(cmd.remove_species)
        conf.save(Host)
